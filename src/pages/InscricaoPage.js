        ...
        <div className="min-h-screen bg-brown-50 flex flex-col">
            ...
            <main className="flex-grow flex items-center justify-center">
                <div className="text-center">
                    <h2 className="text-4xl font-bold mb-4 text-brown-800">Obrigado!</h2>
                    <p className="text-xl text-brown-600">{mensagem}</p>
                </div>
            </main>
        </div>
    );
        ...
        <section id="inscricao-form" className="py-12 md:py-20">
            <div className="container mx-auto px-4">
                <div className="max-w-2xl mx-auto text-center">
                  <h2 className="text-4xl font-bold mb-4 text-brown-800">Formulário de Inscrição</h2>
                ...
                <form onSubmit={handleSubmit} className="max-w-2xl mx-auto p-8 bg-white rounded-2xl shadow-lg">
                  ...
                  <div className="mb-4"><input type="text" name="endereco" placeholder="Endereço completo" value={formData.endereco} onChange={handleChange} required className="w-full p-3 rounded-lg border border-brown-200 focus:outline-none focus:ring-2 focus:ring-brown-500" /></div>
                  <div className="mb-4"><input type="text" name="tipo_produto" placeholder="Qual seu tipo de produto?" value={formData.tipo_produto} onChange={handleChange} required className="w-full p-3 rounded-lg border border-brown-200 focus:outline-none focus:ring-2 focus:ring-brown-500" /></div>
                  <div className="mb-8"><input type="text" name="nome_loja" placeholder="Nome da sua loja ou marca" value={formData.nome_loja} onChange={handleChange} required className="w-full p-3 rounded-lg border border-brown-200 focus:outline-none focus:ring-2 focus:ring-brown-500" /></div>

                  <div className="text-center mt-8">
                    <button type="submit" disabled={status === 'enviando'} className="bg-brown-600 text-white font-bold py-4 px-12 rounded-full text-lg hover:bg-brown-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                      <Send className="w-5 h-5 mr-3" />
                      {status === 'enviando' ? 'Enviando...' : 'Enviar Inscrição'}
                    </button>
                  </div>
                  {status === 'erro' && <p className="text-red-500 text-center mt-4">{mensagem}</p>}
                </form>
            </div>
        </section>
      </main>
